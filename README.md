...
- FastAPI REST API와 SQLite 로컬 DB; PostgreSQL용 Compose 기반은 Phase 2 검증 중
...
Compose는 PostgreSQL 16, Backend, Flutter Web과 PostgreSQL 영구 volume으로 구성됩니다. 로컬 SQLite 실행 방식은 위 Backend 절차 그대로 유지됩니다.

```powershell
Copy-Item .env.example .env
# .env의 POSTGRES_PASSWORD, DATABASE_URL, JWT_SECRET_KEY, KIP_ADMIN_PASSWORD를 변경
...
기본 호스트 포트는 PostgreSQL `5433`, Backend `8000`, Flutter `5001`이며 `.env`에서 변경할 수 있습니다. 종료만 할 때는 `docker compose down`, 개발용 PostgreSQL 데이터까지 제거할 때만 `docker compose down -v`를 사용합니다.

현재 점검 환경에는 Docker CLI가 설치되어 있지 않아 `config/build/up/health`를 실제 실행하지 못했습니다. Compose 구조와 YAML만 정적으로 확인했으며 Docker 또는 PostgreSQL 실행 완료로 간주하지 않습니다. 상세 절차와 별도 integration 명령은 [Docker 안내](docker/README.md)와 [Phase 2 PostgreSQL Checklist](docs/PHASE2_POSTGRES_CHECKLIST.md)를 참고하십시오.
